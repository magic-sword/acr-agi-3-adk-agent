...
def instruction(state: str, schema: dict) -> str:
    if state == 'DECIDE':
        return (
            ...
            'For reusable causal knowledge use causal_memory: read old evidence, or operation=update '
            'with transitions and induce=true to learn rules from observed true/false symbols. '
            'Use stable ground symbols such as gate_open or at(player,exit); omit unobserved facts. '
            'The archive retains final level/WIN evidence across runs. stage_clear is labeled only by the host. '
            'When enough parts are known, call plan_backward with current symbols and observation_id. '
            'A plan is conditional on these models: execute only its next action, then observe. '
            'knowledge_gap gives missing prerequisites to investigate; search_limit is not proof of a gap. '
            'You can update examples and learn in one tool call, plan in the next, then submit. '
            'Call causal_memory and plan_backward individually, never in the same response; inspect the result first. '
