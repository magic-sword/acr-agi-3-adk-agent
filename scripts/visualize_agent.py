...
def render_svg(data: dict) -> str:
    ...
    rows = {state: data['effective_skills'].get(state, []) for state in data['nodes'] if state != 'START'}
    used = {sid for ids in rows.values() for sid in ids}
    connected = {sid: name for sid, name in data['skill_names'].items() if sid in used}
    legend_y = table_y + 80 + len(rows) * 35
    height = legend_y + len(connected) * 26 + 100
    ...
    for r, (state, state_skills) in enumerate(rows.items()):
        y = table_y + 65 + r*35
        text(30, y+5, state, 15)
        for i, sid in enumerate(ids):
            color = '#2563eb' if sid in state_skills else '#e2e8f0'
            parts.append(f'<circle cx="{290+i*cell}" cy="{y}" r="8" fill="{color}"><title>{escape(state+": "+data["skill_names"][sid]+(" connected" if sid in state_skills else " not connected"))}</title></circle>')
    for i, (sid, name) in enumerate(connected.items()):
        text(30, legend_y+i*26, f'{sid}  {name}', 15)
    y = legend_y + len(connected)*26
...
def main():
    ...
    html += '<p>OBSERVEは観測・境界・予算を確認し、DECIDEが振り返りと次の一手をまとめて提出します。COMMITが記憶と操作を確定します。図は現在の3段階と接続中の5スキルに絞り、内部処理は詳細へ分けています。</p>'
    ...
    html += '<details><summary>内部処理・未接続スキルを含む全定義</summary><pre>' + escape(json.dumps(
        {k: data[k] for k in ('internal', 'skill_names', 'effective_skills')}, ensure_ascii=False, indent=2)) + '</pre></details>'
