...
async def check():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        agent = LlmAgent(name='skill_check', model=model, include_contents='none',
            instruction='Verify the connection using the relevant skill in the supplied catalog. Do not guess verification values.',
        ...
        try:
            ...
            passed = (failure is None and marker in '\n'.join(texts)
                      and all(name in calls for name in ('load_skill', 'load_skill_resource')))
