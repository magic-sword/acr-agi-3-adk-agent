...
The three-node loop also exposes `causal_memory` for persistent symbolic examples/rules and `plan_backward` for bounded goal regression. Two bundled skill scripts learn grounded conditions from positive/negative examples and return either a model-based plan or missing conditions for experiments. SQLite files live in `CAUSAL_MEMORY_DIR` (default `outputs/causal-memory`); benchmarks use isolated stores. This is a restricted symbolic learner, with LLM-provided interpretations, not a demonstrated autonomous game solver. See the [design](docs/causal-learning-and-backward-planning-ja.md) and [validation results](docs/local-evaluation-causal-planning-ja.md). Run the synthetic persistence/planning demo with `docker compose run --rm --no-deps dev python scripts/demo_causal_planning.py`.
