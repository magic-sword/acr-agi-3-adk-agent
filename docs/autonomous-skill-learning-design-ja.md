...
追補：[指示とツールの分離に関する調査](prompt-scope-review-ja.md)では、通常判断に学習の詳細指示が混在する実装を見直し、仕事ごとにプロンプトを分ける案を検討した。実行制御を単純に保つことと、一つのLLM呼び出しにすべての指示を渡すことは区別する。指示分離と共有記憶は[攻略ノートの実装](notebook-memory-design-ja.md)で具体化した。以下は当初の設計判断の記録であり、現在の役割構成は実装文書を参照する。
