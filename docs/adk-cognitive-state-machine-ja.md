...
ここでのスキルは「特定の判断を行う指示・入力・出力・検証器」の単位。現在は状態に対応したSkillToolsetを登録し、モデルがツール呼び出しで必要な指示とリソースを読み込む。関数ノードの検証・予算管理はホストが実行する。

ADKには`SkillToolset`と`SKILL.md`を使うスキル機能があるが、公式ではexperimentalとされ、ロード等にツールを使う。現在の`LocalVisionLlm`はツール往復に対応し、同じスキル契約をADKのネイティブスキルへ包装済み。状態が利用可能なスキル集合を制限し、LLMが必要な本文・参照資料をロードする。[実装と検証](adk-native-skills-ja.md)。[公式: Skills](https://adk.dev/skills/)
