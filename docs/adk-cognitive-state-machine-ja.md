...
ログの`trace`と`transitions`もこの3段階だけに揃える。内部処理は`internal_trace`へ分離する。構造図も現在の3段階と接続中スキルのみを表示し、全定義は折りたたみ詳細に残す。
...
```json
{
  "action": {"action": "UP", "reason": "RIGHTで変化がなかったため、UPの作用を一つ試す。"},
...
`action.reason`には、観測根拠とその操作を選ぶ理由を短く出力するよう指示する。ロードしたスキルが選択に役立った場合は、その名称も要約に含める。互換性のため必須契約は増やさず、未出力は分析時に「未記録」として件数を報告する。モデル内部の思考過程を取得・再現する仕組みではない。
...
ローカルCompose実行では既定で`outputs/cognition`へ保存する。`COGNITION_LOG_DIR=`を明示すると無効。直接Runtimeを作成する場合は`log_dir`を指定する。

```bash
make eval-model GAME=ls20 STEPS=20
python3 scripts/analyze_agent.py outputs/cognition
```

生成される`decisions.html`で、各手の状態遷移・判断理由・振り返り・予測・作業メモ・実際のスキルロードと成否を確認できる。ベンチマークでは各ゲームの`cognition`ディレクトリを指定する。

- `<run>.jsonl`: 確定した各手。`previous_outcome`が前のdecision_id・予測・前後観測ID・画面変化を結ぶ。境界をまたぐ比較はnullとし、自然言語予測の正誤は自動判定しない。
- `<run>.tools.jsonl`: ADKの実行前・実行後・例外コールバックによる即時追記。時刻・run_id・観測ID・モデル試行番号・tool_call_id・引数・成否・所要時間・結果を記録する。最終submit_decisionの受理結果も次のHTTP要求なしで記録する。画像本体を除外し、長い本文はプレビュー・文字数・ハッシュへ縮める。
- `<run>.model.jsonl`: モデル試行、応答、利用可能スキルと実際の`tool_executions`。利用可能リストだけを使用の証拠にしない。
- `<run>.observations.jsonl`と`frames/`: 要約の根拠を確認する実観測。

ツールの開始のみで終了がなければ、結果は未確認。ロード失敗をスキル利用成功として集計しない。スキルロード成功も手法を正しく適用した証明ではなく、実際の理由・予測・結果と照合する。確定ログの操作はドライバへの提出であり、実行の確認には次の観測またはゲートウェイ応答を使う。旧ログにない判断理由や実行結果を後から推測で補わない。
