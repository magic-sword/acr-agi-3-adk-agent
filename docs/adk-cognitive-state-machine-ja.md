...
S01は視覚的な問いに応じた証拠の選び方を扱う任意スキル。観測ツールはそのロード有無に関係なく、VERIFY・PLAN・PROBE・REVISEに直接公開する。
...
PLAN／REVISEは `check_plan_order` で仮の操作順の前提条件を前向きに検査できる。結果はモデルが供給した条件の整合性だけを表し、実際のゲーム効果の証拠にはしない。
...
これらは最終回答テキストとしてではなく、状態専用の完了ツールの引数として提出する。

|状態|完了ツール|提出内容|
|---|---|---|
|PLAN|`submit_plan`|計画・具体的な探索依頼・完成した実験のいずれか|
|REVISE|`submit_revision`|修正計画・探索依頼・実験のいずれか|
|PROBE|`submit_experiment`|合法操作と識別可能な実験|
|VERIFY|`submit_interpretation`|結果の解釈|

各LlmAgentは観測・スキル取得を複数回実行できる。完了ツールはスキーマ、観測ID、記憶版、根拠、操作、実験や計画の整合性を隔離した記憶上で検査する。不正な提出はエラーをツール結果として返し、同じモデル判断内で訂正できる。受理時だけADKの`skip_summarization`でツールループを終了し、保持した型付き結果をWorkflowへ返す。追加の最終JSONは不要で、テキストだけの回答は完了と認めない。

完了ツールのActionスキーマは現在の観測にある利用可能ボタンへ絞る。ホストの合法性検査も維持する。モデルログの`submissions`に受理・拒否とその理由を保存するため、最後の枠で拒否されて次のHTTP要求がない場合にも原因を追跡できる。

状態遷移と操作実行の責任はWorkflow／ACT／COMMITに残す。完了ツール自身はゲームを進めない。並行したカーソル変更や二重提出を防ぐため、完了ツールと他の呼び出しを同じ応答に含めた場合は、どちらも実行せず拒否する。
...
最後のHTTP枠では状態専用の完了ツールだけを公開し、`tool_choice=required`を指定する。`tool_choice=none`でテキスト回答へ切り替える方式は廃止した。モデルが制約を無視して追加の情報取得を要求した場合は実行せず、残り予算内での再判断または停止とする。最終枠でも提出の検証は省略しない。
...
状態の必須課題と提出契約は `cognition/instructions.py`、専門手順は `skills/*/SKILL.md` に置く。名前と用途だけを初回に提示し、必要時だけ本文・資料をロードする。旧S13/S15の確定・予算制御はホスト責務であり、13個の任意スキルに含めない。[現行スキル仕様](adk-cognitive-skills-ja.md)。

ノード間の一時作業データ・提出結果はRuntimeが保持する。ADKセッションには確定した認知記憶を公開するが、プロセス障害からの自動再開は対象外。再開対応時は作業データ・証拠参照を復元可能なノード入出力や状態へ移す。
