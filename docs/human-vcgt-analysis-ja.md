...
2026-09-24に設計・実装を改訂した。分析結果は「最初の画像から目標や作用を認識できること」を要求していない。r11l・ls20は達成条件の再解釈、vc33は操作後の変化、s5i5は過去の証拠の参照、su15は目的による評価の違いを示唆する。

そのためOBSERVEは画像・操作・時点の記録を担当し、VERIFY・PLAN・PROBE・REVISEが共通の観測取得スキルで証拠を読み、各自の問いに応じて解釈する。画素差分は受付時に計算できるが、その意味・原因・有用性は別の判断である。初回は検証対象がなく、目標未確定でもPLANで仮説と探索の必要性を検討できる。

この分担は人間の思考順序を実証した結論ではなく、注釈分析から導いた設計仮説。観測と予測の対応、未確認と反証の区別、意思決定に効く未知の選択を評価する。具体的な契約は[改訂ステートマシン設計](adk-cognitive-state-machine-ja.md)を参照。

優先事項は、証拠を持ち越すこと、仮説に適用範囲と反証を持たせること、部分目標に連動・時間・中断条件を加えること。その後、元フレームと対応付けた注釈で判断を評価する。状態理解・作用予測・目標選択・修復は区別するが、別々のモデルやエージェントへ分ける必要はない。
